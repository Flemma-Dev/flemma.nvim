describe("flemma.readiness", function()
  ...
  describe("Suspense + is_suspense", function()
    ...
    it("tostring produces a human-readable label for log output", function()
      local boundary = readiness.get_or_create_boundary("ts", function(done)
        done()
      end)
      local s = readiness.Suspense.new("Waiting for secrets…", boundary)
      assert.equals("Suspense(Waiting for secrets…)", tostring(s))
    end)
