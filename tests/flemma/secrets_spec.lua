...
describe("secrets config defaults", function()
  it("provides gcloud.path = 'gcloud' after bare setup", function()
    local defaults = require("flemma.config")
    assert.is_not_nil(defaults.secrets)
    assert.is_not_nil(defaults.secrets.gcloud)
    assert.equals("gcloud", defaults.secrets.gcloud.path)
  end)

  it("preserves user-supplied gcloud path through vim.tbl_deep_extend", function()
    local defaults = require("flemma.config")
    local user_opts = { secrets = { gcloud = { path = "/nix/store/xyz/bin/gcloud" } } }
    local merged = vim.tbl_deep_extend("force", {}, defaults, user_opts)
    assert.equals("/nix/store/xyz/bin/gcloud", merged.secrets.gcloud.path)
  end)
end)
