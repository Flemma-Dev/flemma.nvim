...
describe("insert-mode colon auto-newline", function()
  ...
  it("reuses existing blank line after role marker instead of inserting a duplicate", function()
    -- Simulates `S` on "@You:" when the buffer already has a blank line after
    -- (the user deletes the line, retypes @You, presses colon)
  ...
  it("treats whitespace-only next line as blank and does not insert a duplicate", function()
    vim.api.nvim_buf_set_lines(bufnr, 0, -1, false, { "@You", "   ", "Hello" })
    vim.api.nvim_win_set_cursor(0, { 1, 4 })
    local handled = keymaps.handle_colon_insert()
    assert.is_true(handled)
