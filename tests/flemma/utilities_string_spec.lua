...
describe("flemma.utilities.string", function()
  ...
  describe("format_cost", function()
    it("formats normal costs with 2 decimal places", function()
      assert.are.equal("$1.50", str.format_cost(1.5))
      assert.are.equal("$0.38", str.format_cost(0.375))
    end)

    it("formats sub-cent costs with 4 decimal places", function()
      assert.are.equal("$0.0050", str.format_cost(0.005))
      assert.are.equal("$0.0010", str.format_cost(0.001))
    end)

    it("formats zero as $0.00", function()
      assert.are.equal("$0.00", str.format_cost(0))
    end)
  end)
