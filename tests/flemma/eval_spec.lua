describe("flemma.templating.eval", function()
  ...
  describe("resolve_include_target", function()
    it("expands ~/path to absolute home directory path", function()
      local home = vim.fn.expand("~")
      local result = eval.resolve_include_target("~/Documents/file.txt", "/some/dirname")
      assert.equals(home .. "/Documents/file.txt", result)
    end)

    it("does not prepend dirname for ~/path", function()
      local home = vim.fn.expand("~")
      local result = eval.resolve_include_target("~/file.txt", "/other/dir")
      assert.equals(home .. "/file.txt", result)
    end)

    it("still resolves relative paths against dirname", function()
      local result = eval.resolve_include_target("./file.txt", "/some/dir")
      assert.equals("/some/dir/file.txt", result)
    end)

    it("still resolves absolute paths as-is", function()
      local result = eval.resolve_include_target("/absolute/path.txt", "/some/dir")
