...
describe("flemma.utilities.po", function()
  ...
  describe("shipped catalogue", function()
    ---@return string path, string content
    local function read_shipped()
      local matches = vim.api.nvim_get_runtime_file("po/flemma.po", false)
      assert.is_truthy(matches[1], "po/flemma.po not found on runtimepath")
    ...
    it("survives an msgcat round-trip with an identical key→content map", function()
      ...
      local path, content = read_shipped()
