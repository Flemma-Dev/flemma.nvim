...
describe("flemma.notify", function()
  ...
  describe("delay", function()
    local function flush_defer(ms)
      vim.wait(ms + 50, function()
        return false
      end)
    end

    it("dispatches after the delay elapses", function()
      notify.delay(50).warn("delayed")
      assert.are.equal(0, #captured)
      flush_defer(50)
      assert.are.equal(1, #captured)
      assert.are.equal("delayed", captured[1].message)
      assert.are.equal(vim.log.levels.WARN, captured[1].level)
    end)

    it("cancel() suppresses a pending notification", function()
      local deferred = notify.delay(50).warn("should not appear")
      deferred.cancel()
      flush_defer(50)
      assert.are.equal(0, #captured)
    end)
    ...
    it("cancel() is idempotent", function()
      local deferred = notify.delay(50).warn("nope")
      deferred.cancel()
      deferred.cancel()
      flush_defer(50)
      assert.are.equal(0, #captured)
    ...
    it("cancel() after the notification fires is a no-op", function()
      local deferred = notify.delay(10).info("already fired")
      flush_defer(10)
      assert.are.equal(1, #captured)
      deferred.cancel()
      assert.are.equal(1, #captured)
    ...
    it("supports all level methods", function()
      notify.delay(10).error("e")
      notify.delay(10).warn("w")
      notify.delay(10).info("i")
      notify.delay(10).debug("d")
      notify.delay(10).trace("t")
      flush_defer(10)
      assert.are.equal(5, #captured)
      assert.are.equal(vim.log.levels.ERROR, captured[1].level)
      assert.are.equal(vim.log.levels.WARN, captured[2].level)
      assert.are.equal(vim.log.levels.INFO, captured[3].level)
      assert.are.equal(vim.log.levels.DEBUG, captured[4].level)
      assert.are.equal(vim.log.levels.TRACE, captured[5].level)
    end)

    it("passes opts through to dispatch", function()
      notify.delay(10).warn("with title", { title = "Custom" })
      flush_defer(10)
      assert.are.equal(1, #captured)
      assert.are.equal("Custom", captured[1].opts.title)
    end)
