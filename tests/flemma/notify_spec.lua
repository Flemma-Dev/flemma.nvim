...
describe("flemma.notify", function()
  ...
  describe("delay", function()
    ...
    it("cancel() after the notification fires is a no-op", function()
      local deferred = notify.delay(50).info("already fired")
      flush_defer(50)
      assert.are.equal(1, #captured)
      deferred.cancel()
      assert.are.equal(1, #captured)
    ...
    it("supports all level methods", function()
      notify.delay(50).error("e")
      notify.delay(50).warn("w")
      notify.delay(50).info("i")
      notify.delay(50).debug("d")
      notify.delay(50).trace("t")
      flush_defer(50)
    ...
    it("passes opts through to dispatch", function()
      notify.delay(50).warn("with title", { title = "Custom" })
      flush_defer(50)
      assert.are.equal(1, #captured)
      assert.are.equal("Custom", captured[1].opts.title)
    end)
