...
describe("flemma.config.proxy", function()
  ...
  describe("write proxy — __newindex records set ops", function()
    ...
    it("records a set op for a nested scalar via sub-proxy navigation", function()
      ...
      assert.equals(1200, store.resolve("parameters.timeout", nil, { is_list = false }))
    end)
    ...
    it("records a set op for a list field (full replace)", function()
      ...
      assert.are.same({ "bash", "grep" }, store.resolve("tools.auto_approve", nil, { is_list = true }))
    end)
    ...
    it("allows table assignment to object field (recursive sub-field writes)", function()
      ...
      assert.equals(500, store.resolve("parameters.timeout", nil, { is_list = false }))
    end)
  ...
  describe("write proxy — DISCOVER callback", function()
    it("DISCOVER callback invoked for unknown key, write succeeds", function()
      ...
      assert.equals("hello", store.resolve("parameters.dynamic_key", nil, { is_list = false }))
    end)
  ...
  describe("list proxy — mutation ops", function()
    it("append records an append op", function()
      ...
      assert.are.same({ "$default", "bash" }, store.resolve("tools.auto_approve", nil, { is_list = true }))
    end)
    ...
    it("remove records a remove op", function()
      ...
      assert.are.same({ "bash" }, store.resolve("tools.auto_approve", nil, { is_list = true }))
    end)
    ...
    it("remove sole item yields an empty list", function()
      ...
      local w = proxy.write_proxy(schema, nil, L.SETUP)
      w.tools.auto_approve:remove("$default")
      assert.are.same({}, store.resolve("tools.auto_approve", nil, { is_list = true }))
    end)
    ...
    it("prepend records a prepend op", function()
      ...
      assert.are.same({ "find", "bash", "grep" }, store.resolve("tools.auto_approve", nil, { is_list = true }))
    end)
  ...
  describe("write proxy with :coerce()", function()
    it("coerces value before validation and recording", function()
      ...
      assert.equals(false, store.resolve("autopilot.enabled", nil, { is_list = false }))
    end)
    ...
    it("coerce runs before validation — invalid raw value becomes valid after coerce", function()
      ...
      assert.equals(true, store.resolve("setting.enabled", nil, { is_list = false }))
    end)
    ...
    it("coerce with optional wrapping", function()
      ...
      local w = proxy.write_proxy(schema, nil, L.SETUP)
      w.feature = false
      assert.equals(false, store.resolve("feature.enabled", nil, { is_list = false }))
    end)
  ...
  describe("hybrid proxy — ObjectNode with allow_list", function()
    ...
    describe("list set via assignment", function()
      it("assigns a sequential table as a list set op", function()
        ...
        local w = proxy.write_proxy(schema, nil, L.SETUP)
        w.tools = { "bash", "grep" }
        assert.are.same({ "bash", "grep" }, store.resolve("tools", nil, { is_list = true }))
      end)
      ...
      it("assigns an empty list", function()
        ...
        local w = proxy.write_proxy(schema, nil, L.SETUP)
        w.tools = {}
        assert.are.same({}, store.resolve("tools", nil, { is_list = true }))
      end)
    ...
    describe("object sub-key assignment via table", function()
      it("non-list table walks into sub-keys (object behavior)", function()
        ...
        local w = proxy.write_proxy(schema, nil, L.SETUP)
        w.tools = { timeout = 300 }
        assert.equals(300, store.resolve("tools.timeout", nil, { is_list = false }))
      end)
    ...
    describe("list methods on sub-proxy", function()
      ...
      it("remove records a remove op at the object path", function()
        ...
        local w = proxy.write_proxy(schema, nil, L.SETUP)
        w.tools:remove("bash")
        assert.are.same({ "grep" }, store.resolve("tools", nil, { is_list = true }))
      end)
      ...
      it("prepend records a prepend op at the object path", function()
        ...
        assert.are.same({ "find", "bash" }, store.resolve("tools", nil, { is_list = true }))
      end)
      ...
      it("method chaining: append then remove", function()
        ...
        assert.are.same({ "grep", "find" }, store.resolve("tools", nil, { is_list = true }))
      end)
    end)
    ...
    describe("object navigation coexists with list ops", function()
      ...
      it("sub-key write via dot navigation works", function()
        ...
        local w = proxy.write_proxy(schema, nil, L.SETUP)
        w.tools.timeout = 500
        assert.equals(500, store.resolve("tools.timeout", nil, { is_list = false }))
      end)
    ...
    describe("buffer isolation", function()
      it("frontmatter list ops are per-buffer", function()
        ...
        assert.are.same({ "bash" }, store.resolve("tools", 1, { is_list = true }))
        assert.are.same({ "grep" }, store.resolve("tools", 2, { is_list = true }))
      end)
    end)
  end)
  ...
  describe("union with list branch", function()
    ...
    it("write proxy returns ListProxy for union with list branch", function()
      ...
      assert.are.same({ "default", "extra" }, store.resolve("policy", nil, { is_list = true }))
    end)
    ...
    it("write proxy: remove on union list works", function()
      ...
      local w = proxy.write_proxy(schema, nil, L.SETUP)
      w.policy:remove("b")
      assert.are.same({ "a", "c" }, store.resolve("policy", nil, { is_list = true }))
    end)
    ...
    it("write proxy: prepend on union list works", function()
      ...
      local w = proxy.write_proxy(schema, nil, L.SETUP)
      w.policy:prepend("z")
      assert.are.same({ "z", "a", "b" }, store.resolve("policy", nil, { is_list = true }))
    end)
    ...
    it("write proxy: set with list table records set op", function()
      ...
      local w = proxy.write_proxy(schema, nil, L.SETUP)
      w.policy = { "only", "these" }
      assert.are.same({ "only", "these" }, store.resolve("policy", nil, { is_list = true }))
    end)
    ...
    it("write proxy: set with function records set op", function()
      ...
      assert.equals(fn, store.resolve("policy", nil, { is_list = true }))
    end)
    ...
    it("write proxy: set with string records set op", function()
      ...
      assert.equals("my.module.path", store.resolve("policy", nil, { is_list = true }))
    end)
    ...
    it("operators work on union list write proxy", function()
      ...
      w.policy = w.policy - "a"
      w.policy = w.policy ^ "z"
      assert.are.same({ "z", "b" }, store.resolve("policy", nil, { is_list = true }))
    end)
