describe("background completion drain", function()
  local hooks_fired = {}
  ...
  before_each(function()
    ...
    vim.api.nvim_create_autocmd("User", {
      pattern = "FlemmaBackgroundCompleted",
      callback = function(ev)
        table.insert(hooks_fired, { name = "background:completed", data = ev.data })
      end,
    })
  end)
  ...
  after_each(function()
    ...
    vim.api.nvim_clear_autocmds({ pattern = "FlemmaBackgroundCompleted" })
  end)
  ...
  it("fires background:completed hook correctly", function()
    local hooks = require("flemma.hooks")
    ...
    assert.equals(1, #hooks_fired)
    assert.equals("background:completed", hooks_fired[1].name)
  ...
  it("drain_and_inject_completions injects results into the buffer", function()
    ...
    for _, item in ipairs(items) do
      injector.append_background_completion(bufnr, item.job_id, item.result)
    end

    local lines = vim.api.nvim_buf_get_lines(bufnr, 0, -1, false)
    local joined = table.concat(lines, "\n")
    assert.truthy(joined:match("%*%*Background Tool Completed:%*%*%s*`bg_drain1`"))
