...
describe("AST and Context", function()
  ...
  it("extends context with variables", function()
    ...
    assert.equals(base.__filename, ext.__filename)
    assert.equals(1, ext.__variables.x)

    local child = ctx.for_include(ext, "/tmp/child.txt")
    assert.equals("/tmp/child.txt", child.__filename)
    assert.is_true(#child.__include_stack >= 2)
  end)

  it("creates eval environment from context", function()
    local base = ctx.from_file("/tmp/flemma.chat")
    base.__variables = { foo = "bar" }
    local env = ctx.to_eval_env(base)
