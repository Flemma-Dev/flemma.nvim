describe("flemma.hl", function()
  ...
  describe(":blend()", function()
    ...
    it("returns parent unchanged when DiffOp delta is nil", function()
      vim.api.nvim_set_hl(0, "TestDiffSrc", { bg = "#aabbcc" })
      vim.api.nvim_set_hl(0, "TestDiffNoAttr", { fg = "#112233" })
      local delta = h.diff("TestDiffSrc", "TestDiffNoAttr", "bg")
      assert.is_nil(delta:delta())
      local result = h.from("TestNormal"):blend("bg", delta):get()
      assert.is_not_nil(result)
      assert.equals("#000000", result.bg)
      assert.equals("#ffffff", result.fg)
      pcall(vim.api.nvim_set_hl, 0, "TestDiffSrc", {})
      pcall(vim.api.nvim_set_hl, 0, "TestDiffNoAttr", {})
    end)

    it("returns parent unchanged when hex mod is invalid", function()
      local result = h.from("TestComment"):blend("bg", "+notahex"):get()
      assert.is_not_nil(result)
      assert.equals("#111111", result.bg)
      assert.equals("#888888", result.fg)
      assert.is_true(result.italic)
  ...
  describe(":omit()", function()
    ...
    it("returns only boolean attrs when all color attrs omitted", function()
      local result = h.from("TestComment"):omit("fg", "bg"):get()
      assert.is_not_nil(result)
  ...
  describe(":contrast()", function()
    ...
    it("returns nil when parent lacks the contrast attr", function()
      local result = h.from("TestCursorLine"):contrast("fg", h.from("TestNormal"):pick("bg"), 4.5):get()
      assert.is_nil(result)
    end)

    it("adjusts bg for contrast against fg", function()
      vim.api.nvim_set_hl(0, "TestLowBg", { bg = "#333333", fg = "#000000" })
      vim.api.nvim_set_hl(0, "TestRefFg", { fg = "#111111" })
      local result = h.from("TestLowBg"):contrast("bg", h.from("TestRefFg"):pick("fg"), 4.5):get()
      assert.is_not_nil(result)
      assert.is_not_nil(result.bg)
      assert.not_equals("#333333", result.bg)
      assert.equals("#000000", result.fg)
      pcall(vim.api.nvim_set_hl, 0, "TestLowBg", {})
      pcall(vim.api.nvim_set_hl, 0, "TestRefFg", {})
    end)

    it("returns nil when against lacks the complement attr", function()
      vim.api.nvim_set_hl(0, "TestSrcFg", { fg = "#888888" })
      vim.api.nvim_set_hl(0, "TestNoComplement", { fg = "#ffffff" })
      local result = h.from("TestSrcFg"):contrast("fg", h.from("TestNoComplement"), 4.5):get()
      assert.is_nil(result)
      pcall(vim.api.nvim_set_hl, 0, "TestSrcFg", {})
      pcall(vim.api.nvim_set_hl, 0, "TestNoComplement", {})
    end)
  end)
  ...
  describe(":merge()", function()
    ...
    it("force strategy overrides non-color attrs", function()
      vim.api.nvim_set_hl(0, "TestSrc", { fg = "#aaaaaa", italic = true })
      vim.api.nvim_set_hl(0, "TestOther", { fg = "#bbbbbb", bold = true })
      local result = h.from("TestSrc"):merge(h.from("TestOther"), "force"):get()
      assert.is_not_nil(result)
      assert.equals("#bbbbbb", result.fg)
      assert.is_true(result.bold)
      assert.is_true(result.italic)
      pcall(vim.api.nvim_set_hl, 0, "TestSrc", {})
      pcall(vim.api.nvim_set_hl, 0, "TestOther", {})
    end)
  end)
