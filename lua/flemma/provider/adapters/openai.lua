...
function M.try_estimate_usage(bufnr, on_result)
  ...
  ---@cast provider flemma.provider.OpenAI

  local endpoint = "https://api.openai.com/v1/responses/input_tokens"
  ...
  if fixture_path then
    ...
    if not api_key then
      local msg = "No API key available for OpenAI."
  ...
  -- The input token endpoint accepts the Responses input shape plus tools,
  -- tool_choice, and reasoning. It rejects generation/transport/cache fields.
  body.stream = nil
  body.store = nil
  ...
  client.send_json_request(request_opts, function(response_body, exit_code, curl_err)
    ...
    if parsed.error then
      on_result({ err = provider:extract_json_response_error(parsed) or "unknown OpenAI error" })
      return
    end
