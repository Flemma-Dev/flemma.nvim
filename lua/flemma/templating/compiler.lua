...
---
--- The generated code uses three environment entries:
  ...
  for i, segment in ipairs(segments) do
    ...
    if segment.kind == "text" then
      ...
      if segment.value and #segment.value > 0 then
        add_line("__emit('" .. escape_lua_string(segment.value) .. "')", lnum)
      end
    ...
    elseif segment.kind == "code" then
      ...
      for j, code_line in ipairs(code_lines) do
        local code_lnum = lnum + j - 1
        add_line(code_line, code_lnum)
      end
    elseif
    ...
    then
      add_line("__emit_part(__segments[" .. i .. "])", lnum)
    end
  end
...
function M.execute(result, env)
  ...
  -- __emit_part: pass-through for structural segments
  ---@param segment flemma.ast.Segment
  ...
  -- Install __emit, __emit_part, __emit_expr_error, __segments on env
  env.__emit = emit
